...
class MainApplication(QtWidgets.QMainWindow, Ui_infogypsies_time.Ui_ExamPeople):
    ...
    def process(self):
        if self.file:
            ...
            if self.file_extension == '.csv':
                ...
                self.df.columns = [n.replace('"', '')
                                   for n in self.df1.columns.str.split(',')[0]]
            ...
            self.people_table.clear()
            self.people_table.setRowCount(0)
            self.lineEdit.setText('No file is selected!')
    ...
    def export(self):
        if self.file:
            ...
            self.people_table.clear()
            self.people_table.setRowCount(0)
            self.lineEdit.setText('No data to export! You push "Cancel" button...')
